...
public class FbsDataFileServiceImpl implements IFbsDataFileService{
	...
	/* (non-Javadoc) 将bd数据插入业务表中，用于下级单位导入
	 * @see nc.itf.export.file.IFbsDataFileService#insertBusinessData(nc.vo.pub.SuperVO[])
	 */
	public void insertBusinessData(SuperVO[] vos) throws BusinessException {
		if (vos == null || vos.length == 0) {
			return;
		}
		String sql = "delete from " + vos[0].getTableName() + " where "
				+ vos[0].getPKFieldName() + " in " + getInSql(vos);
		getDao().executeUpdate(sql);
		
		getDao().insertVOArrayWithPK(vos);
	}
	
	public String getInSql(SuperVO[] vos) throws BusinessException {
		...
		if (pks != null && pks.length > 0) {
			...
			try {
				...
				sTemp1 = dmoTempTbl.insertTempTable(pks, " compare", " pk_id ");
				return sTemp1;
			} catch (Exception e) {
				e.printStackTrace();
				throw new BusinessException(e);
